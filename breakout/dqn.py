...
from torch.serialization import LONG_SIZE
...
class Network(nn.Module):
    ...
    def load(self, load_path):
        ...
        params = {k: torch.as_tensor(v.copy(), device=self.device) for k, v in params_numpy.items()}
...
if __name__ == '__main__':
    ...
    vec_env = DummyVecEnv([make_env for _ in range(NUM_ENV)])
    ...
    for step in itertools.count():
        ...
        if step % 100000 == 0:
            print('Saving model...')
            tracking_save_path = f'./models/atari_model_{step // 100000}.pack'
            online_net.save(tracking_save_path)
